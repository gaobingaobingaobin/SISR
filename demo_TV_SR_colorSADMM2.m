...
dr = d;
dc = d;
y = y(1:dr:end,1:dc:end);%如果是girl.tif,那么就是65*64，如果是lena.tif,那么就是128*128
...
%         X = ifft2(FX);
%         %% %%%%%%%%%%%%%%%%%%%%%%%%%% update D gaobin%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         DhX = ifft2(FX.*FDH);%这三个，如果放在循环外，实验效果更好些。
%         DvX = ifft2(FX.*FDV);
%         HX = ifft2(FB.*FX);
%         D1 = D1 + alpha*(HX-U1);
%         D2 = D2 + alpha*(DhX-U2);
...
%     'edgeWidth', 2);
% % imwrite(im_out, '.\test.tif'); %不更改magnifyOnFigure的参数，不能写入局部放大图
